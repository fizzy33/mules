...
object DispatchOneCache {
  ...
  /**
    ...
    **/
  def ofSingleImmutableMap[F[_]: Concurrent: Sync, K, V](
    defaultExpiration: Option[TimeSpec]
  ): F[DispatchOneCache[F, K, V]] = {
    Concurrent[F].map(Ref.of[F, Map[K, DispatchOneCacheItem[F, V]]](Map.empty[K, DispatchOneCacheItem[F, V]])) { ref =>
      new DispatchOneCache[F, K, V](
  ...
  def ofShardedImmutableMap[F[_]: Concurrent : Sync, K, V](
    shardCount: Int,
    defaultExpiration: Option[TimeSpec]
  ): F[DispatchOneCache[F, K, V]] =
    Concurrent[F].map(MapRef.ofShardedImmutableMap[F, K, DispatchOneCacheItem[F, V]](shardCount)) { mapRef =>
      new DispatchOneCache[F, K, V](
        mapRef,
        None,
        defaultExpiration,
  ...
  def ofConcurrentHashMap[F[_]: Concurrent: Sync, K, V](
    defaultExpiration: Option[TimeSpec],
